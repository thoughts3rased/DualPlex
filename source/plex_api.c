...
// --- Async queue fill ----------------------------------------------------
// Fetches the remainder of an album/playlist's tracks (past whatever was
// already paged in for display) in the background, so a queue built from a
// partially-loaded list gets completed without blocking playback that's
// already started. Same non-blocking curl_multi pattern as the lyrics fetch
// above, but a single request instead of a two-stage lookup.

static CURLM* s_qf_multi = NULL;
static CURL* s_qf_easy = NULL;
static struct curl_slist* s_qf_headers = NULL;
static HttpBuffer s_qf_buf = {0};
static bool s_qf_active = false;
static bool s_qf_done = false;
static int s_qf_start = 0;
static int s_qf_count = 0;

static void qf_cleanup_request(void) {
    if (s_qf_multi && s_qf_easy) {
        curl_multi_remove_handle(s_qf_multi, s_qf_easy);
        curl_easy_cleanup(s_qf_easy);
        s_qf_easy = NULL;
    }
    if (s_qf_headers) {
        curl_slist_free_all(s_qf_headers);
        s_qf_headers = NULL;
    }
    if (s_qf_buf.data) {
        free(s_qf_buf.data);
...
void plex_api_queue_fill_async_start(const char* key, int start, int count) {
    ...
    if (!s_initialized || !key || !key[0] || count <= 0) return;

    char endpoint[512];
    if (strstr(key, "?")) {
        snprintf(endpoint, sizeof(endpoint), "%s&X-Plex-Container-Start=%d&X-Plex-Container-Size=%d", key, start, count);
    } else {
        snprintf(endpoint, sizeof(endpoint), "%s?X-Plex-Container-Start=%d&X-Plex-Container-Size=%d", key, start, count);
    }

    if (!s_qf_multi) s_qf_multi = curl_multi_init();
    s_qf_easy = curl_easy_init();
    if (!s_qf_easy) return;

    char url[PLEX_MAX_URL];
    snprintf(url, sizeof(url), "%s%s", s_server_url, endpoint);

    s_qf_buf.data = malloc(1);
    s_qf_buf.size = 0;
    s_qf_buf.capacity = 1;
    if (s_qf_buf.data) s_qf_buf.data[0] = 0;

    s_qf_headers = curl_slist_append(NULL, "Accept: application/json");
    char token_header[256];
    snprintf(token_header, sizeof(token_header), "X-Plex-Token: %s", s_auth_token);
    s_qf_headers = curl_slist_append(s_qf_headers, token_header);
    s_qf_headers = curl_slist_append(s_qf_headers, client_id_header());
    s_qf_headers = curl_slist_append(s_qf_headers, "X-Plex-Product: " PLEX_PRODUCT);

    curl_easy_setopt(s_qf_easy, CURLOPT_URL, url);
    curl_easy_setopt(s_qf_easy, CURLOPT_HTTPHEADER, s_qf_headers);
    curl_easy_setopt(s_qf_easy, CURLOPT_WRITEFUNCTION, http_write_cb);
    curl_easy_setopt(s_qf_easy, CURLOPT_WRITEDATA, (void*)&s_qf_buf);
    curl_easy_setopt(s_qf_easy, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(s_qf_easy, CURLOPT_CONNECTTIMEOUT, 6L);
    curl_easy_setopt(s_qf_easy, CURLOPT_TIMEOUT, 15L);
    curl_easy_setopt(s_qf_easy, CURLOPT_SSL_VERIFYPEER, 0L);
    curl_easy_setopt(s_qf_easy, CURLOPT_SSL_VERIFYHOST, 0L);

    curl_multi_add_handle(s_qf_multi, s_qf_easy);
    s_qf_start = start;
    s_qf_count = count;
    s_qf_active = true;
    s_qf_done = false;
...
void plex_api_queue_fill_async_update(void) {
    if (!s_qf_active || s_qf_done) return;
    if (!s_qf_multi || !s_qf_easy) return;

    int running = 0;
    CURLMcode mres = curl_multi_perform(s_qf_multi, &running);
    if (mres == CURLM_OK && running > 0) return; // still in flight, check again next frame

    s_qf_done = true; // request finished, one way or another - take_result() sorts out success/failure
}

bool plex_api_queue_fill_async_is_done(void) {
    return s_qf_active && s_qf_done;
}

int plex_api_queue_fill_async_take_result(PlexTrack* out, int max) {
    if (!s_qf_active || !s_qf_done) return 0;

    int parsed = 0;
    if (out && s_qf_easy) {
        long http_code = 0;
        curl_easy_getinfo(s_qf_easy, CURLINFO_RESPONSE_CODE, &http_code);
        if (http_code == 200 && s_qf_buf.data && s_qf_buf.size > 0) {
            parsed = parse_tracks_from_json(s_qf_buf.data, out, s_qf_start, max, NULL);
            LOG_INFO("Async queue fill: fetched %d tracks starting at %d", parsed, s_qf_start);
        } else {
            LOG_WARN("Async queue fill failed: http=%ld", http_code);
        }
    }
